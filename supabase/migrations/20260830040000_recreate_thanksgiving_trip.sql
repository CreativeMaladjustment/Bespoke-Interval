-- Fix: after 20260830030000 ran, public.trips ended up completely empty —
-- including the thanksgiving-london-2026 row that 20260830010000 was
-- supposed to have created. Every seed insert in these migrations is written
-- as `insert ... select ... from public.trips t, (values ...) where
-- t.slug = '...'`, so with no matching trips row, those inserts silently
-- match zero rows and write nothing — no error, just empty tables.
--
-- This migration is self-contained: it (re)creates the trip row itself
-- rather than assuming an earlier migration already did, so it doesn't
-- depend on whatever left trips empty.
--
-- The reseed below is guarded to run only if this trip has no travelers
-- yet. Once real data exists (either from a successful earlier seed, or
-- from someone editing it through the app), this migration must not
-- clobber it back to placeholder content.
